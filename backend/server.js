...
// ── CORS ──────────────────────────────────────────────────────────────────────
const allowedOrigin = (process.env.FRONTEND_URL || 'https://full-stack-inventory-production.up.railway.app').trim();
...
// ── Customers — returns ALL registered clients for admin dashboard ─────────────
app.get('/api/customers', async (req, res) => {
  try {
    const [rows] = await pool.query(
      `SELECT id, name, name AS companyName, email, phone, role
  ...
  try {
    ...
    const result = (products || []).map(p => ({
      ...
      customerPricing:   Object.fromEntries(
        (assignments || []).filter(a => a.productId === p.id).map(a => [a.customerId, Number(a.price)])
      ),
    }));
    res.json(result);
  } catch (err) {
...
app.listen(PORT, '0.0.0.0', () => console.log(`🚀 Gateway Live on Port ${PORT}`));